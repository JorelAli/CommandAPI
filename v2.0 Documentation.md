...
* [CommandExecutor](https://github.com/JorelAli/1.13-Command-API/blob/master/v1.8%20Documentation.md#commandexecutor)
  ...
  * [ProxiedCommandSenders](https://github.com/JorelAli/1.13-Command-API/blob/master/v1.8%20Documentation.md#proxiedcommandsenders)
...
* Download the v2.0 CommandAPI.jar from the download page [here](https://github.com/JorelAli/1.13-Command-API/releases/tag/v2.0)
...
* `verbose-outputs` - Outputs command registration and unregistration information in the console
* `create-dispatcher-json` - Creates a `command_registration.json` file showing a tree of registered commands
...
## Building the CommandAPI (For whoever feels like it)

* Clone the repository however you normally clone a repository 

  `https://github.com/JorelAli/1.13-Command-API.git`

* Create a folder `/spigotlibs/` in the same directory as the `pom.xml` file

* Copy spigot.jar **server** files into this folder. Spigot.jar files can be attained using Spigot's [BuildTools](https://www.spigotmc.org/wiki/buildtools/). The jar files must be the jar files normally found in `Spigot/Spigot-Server/target/`.
