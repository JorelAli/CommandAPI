# Documentation for v1.3 [IN PROGRESS]
...
- Create a custom `ArgumentType` to handle things from a list
  - Support for doing so is ~~very limited~~ impossible, as shown by _Luck_ in [this thread comment](https://www.spigotmc.org/threads/commodore-utility-for-using-minecrafts-1-13-brigadier-library-in-bukkit-plugins.329682/#post-3152561)
- Register a command multiple times for each element in the list
  - This is the method `/gamemode` uses, however it's only effective for small lists
    In short, /gamemode is 4 separate commands:
	- /gamemode survival
	- /gamemode creative
	- /gamemode adventure
	- /gamemode spectator
	
	as opposed to 1 command:
	- /gamemode [gamemode]
  - Large lists (for example, the `Material` enum) creates a large amount of entries in the command registry
  - If a command had two large lists, the space growth rate is `O(n²)`, which could prevent a client from logging into a server
